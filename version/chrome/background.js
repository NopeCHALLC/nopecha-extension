(() => {
    ...
    class Net {
        static async fetch({tab_id, data: {url, options}}) {
            try {
    ...
    class Tab {
        ...
        static info({tab_id}) {
            return new Promise(resolve => {
                try {
                    chrome.tabs.get(tab_id, tab => resolve(tab));
                } catch (e) {
    ...
    class Settings {
        ...
        static _save() {
            return new Promise(resolve => chrome.storage.sync.set({settings: Settings.data}, resolve));
        }
